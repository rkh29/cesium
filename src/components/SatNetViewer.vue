...
// 路由阈值（对应文档物理约束）
const D2G_DIST = 0.8; // LEO→地面视距（D2G 优先）
const ISL_DIST = 0.85; // 跨轨 ISL 开闭阈值（赤道孤岛效应）
const GEO_FOV = 6.2; // GEO 端机视场（EDRS 模型：每 GEO 3 端机）
...
function initScene(el: HTMLDivElement) {
  ...
  RAANS.forEach((raanDeg, pi) => {
    ...
    for (let i = 0; i < SATS_PER_PLANE; i++) {
      ...
      const sat = new THREE.Mesh(
        satGeo,
        new THREE.MeshBasicMaterial({ color: PLANE_COLORS[pi] }),
      );
    ...
    ig.setAttribute(
      "position",
      new THREE.BufferAttribute(new Float32Array(SATS_PER_PLANE * 3), 3),
    );
    const il = new THREE.LineLoop(
      ig,
      new THREE.LineBasicMaterial({
        color: PLANE_COLORS[pi],
        transparent: true,
  ...
  const d2dMesh = new THREE.LineSegments(
    d2gGeo,
    new THREE.LineBasicMaterial({
      color: 0x33ff66,
      transparent: true,
  ...
  const relMesh = new THREE.LineSegments(
    relayGeo,
    new THREE.LineBasicMaterial({
      color: 0xff5533,
      transparent: true,
  ...
  const islMesh = new THREE.LineSegments(
    islGeo,
    new THREE.LineBasicMaterial({
      color: 0xbb44ff,
      transparent: true,
  ...
  function animate() {
    ...
    leoSats.forEach((sd) => {
      ...
      sd.mesh.position.set(
        R_LEO * Math.cos(sd.angle),
        0,
        -R_LEO * Math.sin(sd.angle),
      );
    });
    ...
    leoSats.forEach((sd) => {
      ...
      const arr = sd.intraLine.geometry.attributes.position
    ...
    leoSats.forEach((sd) => {
      ...
      let bestGS: THREE.Mesh | null = null,
        bestD = D2G_DIST;
      gsMeshes.forEach((gs) => {
...
onMounted(() => {
  if (containerRef.value) initScene(containerRef.value);
});
onUnmounted(() => {
...
<template>
  <div class="sat-wrapper">
    ...
    <div class="hud hud-topo">
      ...
      <div class="hud-section">
        ...
        >
          <span
            class="plane-dot"
            :style="{ background: ['#33ccff', '#44ff99', '#ffaa33'][i] }"
          ></span>
      ...
      <div class="hud-section">
        ...
        <div
          v-for="(g, i) in ['GEO-A  0°', 'GEO-B 120°', 'GEO-C 240°']"
          :key="i"
          class="hud-plane-row"
        >
          <span
            class="plane-dot"
            style="background: #ff4444; border-radius: 50%"
          ></span>
    ...
    <div class="legend">
      <div class="leg-group">
        <div class="leg-row">
          <span class="ldot" style="background: #ffcc00"></span>MCS 主控站
        </div>
        <div class="leg-row">
          <span class="lseg" style="background: #ffcc00; opacity: 0.5"></span
          >地面光缆汇聚
        </div>
      </div>
      ...
      <div class="leg-group">
        <div class="leg-row">
          <span class="lseg" style="background: #33ff66"></span>D2G 直连地面
        </div>
        <div class="leg-row">
          <span class="lseg" style="background: #ff5533"></span>盲区→GEO 中继
        </div>
        <div class="leg-row">
          <span class="lseg" style="background: #bb44ff"></span>跨轨交织 ISL
        </div>
        <div class="leg-row">
          <span class="lseg" style="background: #ff3333; height: 3px"></span>GEO
          骨干闭环
        </div>
        <div class="leg-row">
          <span class="lseg" style="background: #44ffaa"></span>GEO-B→MCS 馈电
        </div>
      </div>
      <div class="leg-divider"></div>
      <div class="leg-group">
        <div class="leg-row">
          <span class="lseg" style="background: #33ccff"></span>同轨骨干环 (面Ⅰ)
        </div>
        <div class="leg-row">
          <span class="lseg" style="background: #44ff99"></span>同轨骨干环 (面Ⅱ)
        </div>
        <div class="leg-row">
          <span class="lseg" style="background: #ffaa33"></span>同轨骨干环 (面Ⅲ)
        </div>
      </div>
    </div>
...
.hud-title {
  ...
  margin-bottom: 2px;
  padding-bottom: 4px;
  border-bottom: 1px solid rgba(142, 220, 255, 0.2);
}
...
.hud-topo {
  ...
  box-shadow: 0 0 12px rgba(142, 220, 255, 0.12);
  min-width: 190px;
}
.hud-section {
  display: flex;
