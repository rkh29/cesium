...
export const useSatelliteStore = defineStore("satellite", () => {
  ...
  const positions = ref<
    Record<string, { latitude: number; longitude: number; altitude: number }>
  >({});
  ...
  const updateSatellitesFromInstances = () => {
    const satelliteInstances = instanceStore.instancesForDisplay.filter(
      (inst) => inst.type.toLowerCase().includes("satellite"),
    );

    satellites.value.forEach((sat) => {
      const inst = satelliteInstances.find((i) => i.id === sat.instanceId);
      if (inst) {
        const relatedLinks = linkStore.linksForDisplay.filter((link) =>
          link.endpoints.includes(inst.id),
        );
        const hasDangerLink = relatedLinks.some(
          (link) => link.status === "danger" || !link.enabled,
        );
        const hasWarningLink = relatedLinks.some(
  ...
  const addSatellite = (sat: Partial<Satellite>) => {
    const newId =
      satellites.value.length > 0
        ? Math.max(...satellites.value.map((s) => s.id)) + 1
        : 1;
