...
export const useInstanceStore = defineStore("instance", () => {
  ...
  ): InstanceDisplay["status"] => {
    if (!resource) return "offline";
    if (resource.cpu_usage > 90 || resource.mem_byte > 8 * 1024 * 1024 * 1024)
      return "danger";
    if (resource.cpu_usage > 75 || resource.mem_byte > 6 * 1024 * 1024 * 1024)
      return "warning";
    return "normal";
  };
  ...
  const runningCount = computed(
    () => instances.value.filter((item) => item.start).length,
  );
  const totalCount = computed(() => instances.value.length);

  const getInstanceById = (id: string) =>
    instances.value.find((item) => item.instance_id === id);
