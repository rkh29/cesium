...
export const useAnomalyStore = defineStore("anomaly", {
  ...
  getters: {
    pendingAnomalies: (state) =>
      state.anomalies.filter((a) => a.status === "pending"),
    fixedAnomalies: (state) =>
      state.anomalies.filter((a) => a.status === "fixed"),
    softwareAnomalies: (state) =>
      state.anomalies.filter((a) => a.anomaly_type === "software"),
    hardwareAnomalies: (state) =>
      state.anomalies.filter((a) => a.anomaly_type === "hardware"),
