...
function toRawEmulationInfo(config: EmulationConfig): RawEmulationInfo {
  return {
    ...
    type_config: Object.fromEntries(
      Object.entries(config.instance_type_config).map(
        ([typeName, typeConfig]) => [
          typeName,
          {
...
function createInitialState(): MockState {
  return {
    ...
    databaseItems: {
      "/emulation_config": JSON.stringify(
        toRawEmulationInfo(defaultEmulationConfig),
      ),
    },
  };
}
...
function buildInstanceDetail(instance: Instance): InstanceDetail {
  ...
  return {
    ...instance,
    image: instance.type.toLowerCase().includes("ground")
      ? "ground-station:mock"
      : "satellite:mock",
    resource_limit: {
      nano_cpu: instance.type.toLowerCase().includes("ground")
        ? 1_000_000_000
        : 2_000_000_000,
...
function normalizeEmulationConfig(value: unknown): EmulationConfig | null {
  ...
  if (
    raw.instance_type_config &&
    typeof raw.instance_type_config === "object"
  ) {
    ...
    Object.entries(raw.type_config).map(([typeName, typeConfig]) => [
      typeName,
      {
        ...
        resource_limit: {
          ...
          memory_byte:
            typeConfig.resource_limit?.memory_byte ?? 512 * 1024 * 1024,
        },
      },
    ]),
  );
...
async function handleMockRequest(
  ...
  const prefix =
    typeof config.params?.prefix === "string"
      ? config.params.prefix
      : undefined;
  ...
  if (method === "post" && path === "/instances") {
    ...
    if (payload?.instance_id) {
      ...
      const index = mockState.instances.findIndex(
        (item) => item.instance_id === nextInstance.instance_id,
      );
      if (index >= 0) {
  ...
  if (method === "delete" && instanceDeleteMatch) {
    ...
    mockState.instances = mockState.instances.filter(
    ...
    mockState.links = mockState.links.filter(
      (link) => !link.connect_instance.includes(instanceId),
    );
  ...
  if (method === "get" && nodeLinksMatch) {
    ...
    return clone(
      mockState.links.filter((link) => link.node_index === nodeIndex),
    );
  }
  ...
  if (method === "post" && path === "/links") {
    ...
    if (
      payload?.link_id &&
      Array.isArray(payload.connect_instance) &&
      payload.connect_instance.length === 2
    ) {
      const nextLink: Link = {
        ...
        connect_instance: [
          payload.connect_instance[0],
          payload.connect_instance[1],
        ],
        node_index: payload.node_index ?? 0,
      };
      const index = mockState.links.findIndex(
        (item) => item.link_id === nextLink.link_id,
      );
      if (index >= 0) {
  ...
  const instanceResourceMatch = path.match(
  ...
  if (method === "get" && path === "/resource/period/instance/all") {
    return Object.fromEntries(
      Object.entries(mockState.instanceResources).map(
        ([instanceId, resource]) => [
          instanceId,
  ...
  const instancePeriodMatch = path.match(
    /^\/resource\/period\/instance\/([^/]+)$/,
  );
  ...
  if (method === "get" && path === "/resource/period/link/all") {
    return Object.fromEntries(
      Object.entries(mockState.linkResources).map(([linkId, resource]) => [
        linkId,
        buildLinkHistory(resource),
      ]),
    );
  }
  ...
  if (method === "get" && path === "/database/items") {
    return Object.fromEntries(
      Object.entries(mockState.databaseItems).filter(
  ...
  if (method === "put" && anomalyUpdateMatch) {
    ...
    const index = mockState.anomalies.findIndex(
