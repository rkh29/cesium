...
const api = axios.create({
  ...
  adapter:
    apiMode === "mock"
      ? createMockAdapter({ wrapApiResponse: true })
      : undefined,
});
...
const rawApi = axios.create({
  ...
  adapter:
    apiMode === "mock"
      ? createMockAdapter({ wrapApiResponse: false })
      : undefined,
});
...
export const instanceApi = {
   ...
   */
  getList: (
    params?: Partial<PageRequest> & { filter?: string; type?: string },
  ): Promise<Instance[]> => api.get("/instances", { params }),
...
export const linkApi = {
  ...
  add: (link: any): Promise<void> => api.post("/links", link),
...
export const nodeApi = {
  getList: (): Promise<Node[]> => api.get("/node/"),
...
export const resourceApi = {
  getInstanceResource: (
    instanceId: string,
  ): Promise<Record<string, InstanceResource>> =>
  ...
  getInstanceResourceHistory: (
    instanceId: string,
    period: string,
  ): Promise<Record<string, TimedInstanceResource[]>> =>
    api.get(`/resource/period/instance/${instanceId}`, { params: { period } }),

  getAllInstanceResourceHistory: (
    period: string,
  ): Promise<Record<string, TimedInstanceResource[]>> =>
  ...
  getLinkResourceHistory: (
    linkId: string,
    period: string,
  ): Promise<Record<string, TimedLinkResource[]>> =>
    api.get(`/resource/period/link/${linkId}`, { params: { period } }),

  getAllLinkResourceHistory: (
    period: string,
  ): Promise<Record<string, TimedLinkResource[]>> =>
    api.get("/resource/period/link/all", { params: { period } }),

  getNodeResource: (
    nodeIndex: number | "all",
  ): Promise<Record<number, HostResource>> =>
    api.get(`/resource/last/node/${nodeIndex}`),

  getNodeResourceHistory: (
    nodeIndex: number | "all",
    period: string,
  ): Promise<Record<number, TimedHostResource[]>> =>
    api.get(`/resource/period/node/${nodeIndex}`, { params: { period } }),
};

export const emulateApi = {
  getConfig: (): Promise<EmulationConfig> => api.get("/emulation/"),
  ...
  updateConfig: async (config: ConfigEmulationRequest): Promise<void> => {
    ...
    Object.entries(config).forEach(([typeName, typeConfig]) => {
      ...
      const nanoCpu = parseResourceLimit(
        typeConfig.resource_limit.nano_cpu,
        "dec",
      );
      const memoryByte = parseResourceLimit(
        typeConfig.resource_limit.memory_byte,
        "bin",
      );
    ...
    // 使用数据库 API 直接写入 etcd
    return databaseApi.update(
      "/emulation_config",
      JSON.stringify(emulationInfo),
    );
  },

  start: (): Promise<void> => api.post("/emulation/start"),

  stop: (): Promise<void> => api.post("/emulation/stop"),

  addTopology: (topology: TopologyRequest): Promise<void> =>
    api.post("/emulation/topology", topology),

  reset: (): Promise<void> => api.post("/emulation/reset"),
};
...
export const platformApi = {
  ...
  getTime: (): Promise<string> => api.get("/platform/time"),
...
export const positionApi = {
   ...
   */
  getAll: (): Promise<PositionMap> => api.get("/position/all"),
...
export const fileApi = {
  ...
  download: (fileName: string): string => `/api/file/download/${fileName}`,
};
...
export const databaseApi = {
  ...
  delete: (key: string): Promise<void> => api.post("/database/delete", { key }),
};
